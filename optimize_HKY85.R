...
# get the frequency of A, T, C, G
nA <- mean(apply(vp_dna_msa_collapsed, 1, function(vpseq){
  sum(str_count(vpseq, pattern = "A"))
}))

nT <- mean(apply(vp_dna_msa_collapsed, 1, function(vpseq){
  sum(str_count(vpseq, pattern = "T"))
}))

nC <- mean(apply(vp_dna_msa_collapsed, 1, function(vpseq){
  sum(str_count(vpseq, pattern = "C"))
}))

nG <- mean(apply(vp_dna_msa_collapsed, 1, function(vpseq){
  sum(str_count(vpseq, pattern = "G"))
}))

nATCG <- c(nA, nT, nC, nG)
pATCG <- nATCG/sum(nATCG)
...
get_corr_product_arbip <- function(pars){ # we use an arbitrary initial matrix
  ...
  pa <- pars[1]
  pb <- 1
  prate <- c(     0, piT*pb, piC*pb, piG*pa,
                  piA*pb,     0, piC*pa, piG*pb,
                  piA*pb, piT*pa,     0, piG*pb,
                  piA*pa, piT*pb, piC*pb,     0)
  prate <- matrix(prate, nrow = 4, byrow = TRUE)
  diag(prate) <- -rowSums(prate)
  a <- pars[2]
  b <- pars[3]
